...
fn main() {
    ...
    let mut body: [Pos; 8] = [(0,0); 8];
    let mut tail: Pos = (0, 0);
    if let Ok(lines) = readlines("./input.txt") {
        for line in lines {
            if let Ok(l) = line {
                process_move(l, &mut head, &mut tail, &mut body, &mut visited);
            }
        }
...
fn adjust_body(head: &Pos, body: &mut [Pos;8], tail: &mut Pos) {
    // Check if each element is in the vicinity of the next element
    // If it's not, then move the it straightly or diagonally
    let mut last_cell = head;
    for cell in body {
        let vic = get_vicinity(last_cell);
        if !vic.contains(cell) {
            // Move it
            let origin = cell.clone();
            *cell = move_once(&origin, last_cell)
        }
        last_cell = &(*cell);
    }
    let vic = get_vicinity(last_cell);
    if !vic.contains(tail) {
        let origin = tail.clone();
        *tail = move_once(&origin, last_cell);
    }
}
...
fn process_move(mv: String, head: &mut Pos, tail: &mut Pos, body: &mut [Pos;8], visited: &mut HashSet<Pos>) {
